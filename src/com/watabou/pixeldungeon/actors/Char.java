...
public abstract class Char extends Actor {
	...
	public boolean attack( Char enemy ) {
		...
		if (hit( this, enemy, false )) {
			damageEnemy(visibleFight, enemy);
			defeatedEnemy(visibleFight, enemy);
			return true;
		} else {
			enemyDefends(visibleFight, enemy);
			return false;
		}
	}

	private void damageEnemy(boolean visibleFight, Char enemy) {
		if (visibleFight) {
			GLog.i( TXT_HIT, name, enemy.name );
		}
	...
	private void defeatedEnemy(boolean visibleFight, Char enemy) {
		if (!enemy.isAlive() && visibleFight) {
			if (enemy == Dungeon.hero) {
	...
	private void enemyDefends(boolean visibleFight, Char enemy) {
		if (visibleFight) {
			String defense = enemy.defenseVerb();
			enemy.sprite.showStatus( CharSprite.NEUTRAL, defense );
			if (this == Dungeon.hero) {
